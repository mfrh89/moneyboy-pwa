...
export const SubscriptionAlert: React.FC<SubscriptionAlertProps> = ({ subscriptions, onItemClick }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-[#f38ba8]/10 to-[#cba6f7]/10 border border-[#f38ba8]/30 rounded-2xl p-4 shadow-lg animate-in slide-in-from-top-2 duration-300">
      <div className="flex items-center gap-2 mb-3">
        <AlertCircle className="w-5 h-5 text-[#f38ba8]" />
        <h3 className="text-sm font-bold text-[#f38ba8] uppercase tracking-wide">
      ...
      <div className="space-y-2">
        {subscriptions.map((sub) => {
          ...
          // Determine which date to show (prioritize cancellation deadline if closer)
          ...
          return (
            <div
              ...
              className="bg-[#1e1e2e]/80 hover:bg-[#1e1e2e] rounded-lg p-3 border border-[#313244] cursor-pointer transition-all hover:border-[#cba6f7]/50 group"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <Repeat className="w-3.5 h-3.5 text-[#cba6f7] flex-shrink-0" />
                    <span className="font-bold text-[#cdd6f4] text-sm truncate">{sub.title}</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-[#a6adc8]">
                    <Calendar className="w-3 h-3" />
                    <span>{relevantDate.label}</span>
                    <span className="text-[#6c7086]">•</span>
                    <span className={timeDiff.isUrgent ? 'text-[#f38ba8] font-bold' : 'text-[#a6adc8]'}>
                      {timeDiff.text}
                    </span>
                  </div>
                </div>
                <div className="text-right flex-shrink-0">
                  <div className="font-mono text-sm font-bold text-[#cdd6f4]">
