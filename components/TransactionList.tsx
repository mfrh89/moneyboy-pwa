  ...
  return (
    <div className="bg-surface-lowest rounded-ds-lg shadow-float overflow-hidden flex flex-col h-fit">
      <div className="p-4 bg-surface-low flex justify-between items-center shrink-0">
        <h3 className={`font-semibold text-[1.25rem] ${accentColor}`}>{title}</h3>
          ...
          <div className="space-y-1">
            {filteredItems.map((item) => (
              <div
                key={item.id}
                onClick={() => onEdit(item)}
                className="group flex items-center justify-between p-3 rounded-ds-md hover:bg-surface-low transition-all cursor-pointer"
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Pencil className="w-3 h-3 text-outline-variant opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
              </div>
            ))}
          </div>
